...
def load_from_file(file_name):
    ...
    # 画像を読み込む
    _image = cv2.imread(filename=file_name)
...
def remove_after_characters(text):
    ...
    modified_text = re.sub(r'\=.*', '', modified_text)
    pattern = re.compile(r'[ァ-ヴー\n]+')
    modified_text = ''.join(pattern.findall(modified_text))
    return modified_text

#  ここより下がMain 関数に当たる、最初に実行される場所

_images_file_names = ["private_sample04.jpg", "private_sample05.jpg"]

for _file_name in _images_file_names:
    image, template = load_from_file(_file_name) # イメージファイルを取得する 
